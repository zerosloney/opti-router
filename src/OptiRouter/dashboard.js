...
async function loadLogs() {
    ...
    try {
        ...
        (data.items||[]).forEach(function(item){
            ...
            tr.innerHTML = '<td>' + new Date(item.timestamp).toLocaleTimeString() + '</td><td>' + (item.model||'') + '</td><td>' + ((item.promptTokens||0)+(item.completionTokens||0)) + '</td><td>$' + (item.cost||0).toFixed(6) + '</td><td>' + (item.latencyMs||0) + 'ms</td><td class="' + (item.success?'success':'failure') + '">' + (item.success?'成功':'失败') + '</td><td>' + (item.isStreaming?'是':'否') + '</td>';
