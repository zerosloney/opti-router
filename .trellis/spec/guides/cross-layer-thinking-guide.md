...
## OptiRouter-Specific Cross-Layer Boundaries

When modifying code that crosses these boundaries, read the relevant spec first:

- [ ] **Policy chain → Endpoints**: `RouterDecision.Reason` string is for debugging only. Use `BudgetExhausted` flag for budget logic. See [routing.md](../backend/routing.md) §3.
- [ ] **Backend API → Frontend**: `ApiService` DTOs must match the API response shape. Add `[JsonPropertyName]` for snake_case fields. See [type-safety.md](../frontend/type-safety.md) §3.
- [ ] **SQLite connections**: Two stores share one DB file with independent connections. `busy_timeout` serializes cross-store writes. See [database-guidelines.md](../backend/database-guidelines.md) §3.
- [ ] **Config JSON → Runtime**: `models-config.json` and `appsettings.json` changes trigger `IOptionsMonitor.OnChange`. Only connection fields (BaseUrl/ApiKey/Timeout) cause client rebuild. See [directory-structure.md](../backend/directory-structure.md) §3.
- [ ] **Blazor pre-render → JS interop**: Canvas chart calls must be guarded by `OnAfterRenderAsync(firstRender)`. See [component-guidelines.md](../frontend/component-guidelines.md) §3.
