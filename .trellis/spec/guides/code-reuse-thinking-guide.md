...
## OptiRouter-Specific Reuse Patterns

When writing code in this repo, reuse these existing abstractions instead of duplicating:

- **Storage interfaces**: `IRequestAuditStore`, `ICostLedgerStore`, `ICircuitStateStore` each have a `Sqlite*` (prod) and `InMemory*` (test) impl. Never write a new store — extend the interface + add both impls. See [database-guidelines.md](../backend/database-guidelines.md) §2.
- **Policy interface**: New routing behavior implements `IRouterPolicy` and registers in the `Program.cs` chain. Do not add routing logic to `ProxyOrchestrator`. See [routing.md](../backend/routing.md) §2.
- **Capability tags**: Vision/tool-use/json-mode constants live once in `ModelCapabilities` — never hardcode strings across policies/validators. See [routing.md](../backend/routing.md) §3.
- **Token estimation**: Use `ITokenEstimator` (`TiktokenTokenEstimator` / `BucketTokenEstimator`), not ad-hoc counting. Do not duplicate BPE logic.
- **Frontend DTOs**: All API DTOs are nested records in `ApiService.cs`. Add API fields there, not in component-local classes. See [type-safety.md](../frontend/type-safety.md) §2.
- **Frontend form state**: Mutable private form classes (`NewModelForm`, `EditModelForm`) for `@bind` — never mutate the record DTOs. See [state-management.md](../frontend/state-management.md) §5.
- **Secrets**: Never log ApiKey/Bearer tokens. Reuse `HasApiKey` bool pattern. See [logging-guidelines.md](../backend/logging-guidelines.md) §7.
