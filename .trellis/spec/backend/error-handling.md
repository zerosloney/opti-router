...
| `ResponseSizeLimitExceededException` | Streaming response exceeded `MaxResponseStreamBytes`/`MaxStreamLineBytes`, or a non-streaming upstream body exceeded the 1 MiB materialization ceiling | 200 (in-band SSE after stream start); normal failover boundary before response start | `Clients/` ns. Carries `LimitBytes`. Mapped to `RESPONSE_TOO_LARGE` for SSE and preserved as the dedicated internal category for non-stream success/error bodies. Use this type, not `InvalidOperationException`. |
